...
// str + 1 如何移动？
// text
// str 数组在栈上连续排列（假设从地址 1000 开始，64位系统每个指针占8字节）：
...
//       +-------+-------+-------+-------+
//       指向     指向     指向     指向
//      "Welcome" "to"  "Fortemedia" "Nanjing"



// 为什么(*p++) + 2移动的是字符个数而不是数组元素个数啊
// 这个问题问得非常精准，直击本质。答案只有一句话：因为 + 运算符的左边是一个 char* 类型的指针，而不是一个指向数组元素的二级指针。

// 🧭 决定移动步长的是指针类型
// 在 C 语言中，指针加法的步长（即移动的字节数）是由指针的类型决定的，而不是由它指向的内容在“语义”上属于哪个数组决定的。

// (*p++) 这个表达式的值是一个 char* 类型的指针。
// 因为它解引用了一个 char**（即 p 指向 char*），所以得到的类型是 char*，也就是指向字符的指针。
// 指针类型是 char*，因此 +2 的步长就是 2 * sizeof(char)，也就是移动 2 个字节（2 个字符位置）。

// 如果你想要“移动数组元素”，你需要的是指针类型为 char**。
// 例如，p 本身是 char**，所以 p + 1 才会从 str[1] 移动到 str[2]，这是因为 char** 类型的指针加 1，会移动 sizeof(char*) 个字节（通常 4 或 8 字节）。

// 🧩 用代码对比更清楚
// c
// char *str[] = {"Welcome", "to", "Fortemedia", "Nanjing"};
// char **p = str + 1;   // p 指向 str[1]

// // 情况1：移动数组元素
// p + 1;         // p 从 str[1] 移动到 str[2]，因为 p 是 char** 类型
//                // 移动了 1 个“数组元素”（指针大小）

// // 情况2：移动字符位置
// (*p) + 2;      // *p 是 char*，指向 "to" 的首字符
//                // +2 移动了 2 个 char，从 't' 到 'o' 再到结束符